...
api.get('/orders', function(req, res) {
  // TODO - Unify the product: '1a2b', account: '2a3b' with their fields in relevant backends
  fh.service({
...
api.use(function(req, res, next){
  return fh.cache({ act : 'load', key : req.path }, function(err, cacheRes){
    if (!err && cacheRes){
...
api.get('/accounts', function(req, res) {
  ...
  return fh.service({
    ...
    return fh.service({
      ...
      fh.cache({ act : 'save', key : req.url, value : JSON.stringify(filtered), expire : EXPIRES}, _.noop);
      return filtered;
    }));
  });
});
